...
function validateSession(
  v: unknown,
): ValidationOk<UploadSession> | ValidationErr {
  if (!isRecord(v))
...
function validateIntervention(
  v: unknown,
): ValidationOk<UploadIntervention> | ValidationErr {
  if (!isRecord(v))
...
function validateOnboardingResponse(
  v: unknown,
): ValidationOk<UploadOnboardingResponse> | ValidationErr {
  if (!isRecord(v))
...
function validateExitSurveyResponse(
  v: unknown,
): ValidationOk<UploadExitSurveyResponse> | ValidationErr {
  if (!isRecord(v))
...
export function validatePayload(
  body: unknown,
): ValidationOk<UploadPayload> | ValidationErr {
  ...
  let onboarding_response: UploadOnboardingResponse | null | undefined =
    undefined;
  ...
  let exit_survey_response: UploadExitSurveyResponse | null | undefined =
    undefined;
