...
// src/db.ts
function getDbConfigFromEnv(env) {
...
async function getCounts(config) {
  const client = new Client({
    host: config.host,
    port: config.port,
    database: config.database,
...
async function exportSessionsCsv(config, sinceMs) {
  const client = new Client({
    host: config.host,
    port: config.port,
    database: config.database,
...
async function exportInterventionsCsv(config, sinceMs) {
  const client = new Client({
    host: config.host,
    port: config.port,
    database: config.database,
  ...
  try {
    await ensureSchema(client);
    const params = [];
    const where = sinceMs ? (params.push(sinceMs), `WHERE intervention_start_ts >= to_timestamp($1 / 1000.0)`) : "";
    const res = await client.query(
      `
      SELECT
        intervention_id,
...
async function insertSession(client, s) {
  await client.query(
    `
...
async function insertIntervention(client, i) {
  await client.query(
    `
...
function requireAdmin(event) {
  ...
  const headerToken = event.headers?.["x-admin-token"] ?? event.headers?.["X-Admin-Token"] ?? event.headers?.["x-admin-token".toUpperCase()];
...
var handler = async (event) => {
  ...
  if (path.startsWith("/admin")) {
    ...
    const dbConfig2 = getDbConfigFromEnv(process.env);
    if (!dbConfig2) return json(500, { ok: false, error: "DB not configured" });
    try {
      if (method === "GET" && path === "/admin/counts") {
        const counts = await getCounts(dbConfig2);
        return json(200, { ok: true, counts });
      }
      if (method === "GET" && path === "/admin/sessions.csv") {
        const since = event?.queryStringParameters?.since_ms;
        const sinceMs = since ? Number(since) : void 0;
        const csv = await exportSessionsCsv(dbConfig2, Number.isFinite(sinceMs) ? sinceMs : void 0);
        return text(200, csv, "text/csv; charset=utf-8");
      }
      if (method === "GET" && path === "/admin/interventions.csv") {
        const since = event?.queryStringParameters?.since_ms;
        const sinceMs = since ? Number(since) : void 0;
        const csv = await exportInterventionsCsv(dbConfig2, Number.isFinite(sinceMs) ? sinceMs : void 0);
        return text(200, csv, "text/csv; charset=utf-8");
      }
      return json(404, { ok: false, error: "Not found" });
    } catch (e) {
